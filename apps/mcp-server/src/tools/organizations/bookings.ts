...
export const getOrgBookingsSchema = {
  ...
  userIds: z.string().describe("Filter bookings by ids of users within your organization.").optional(),
};
...
export async function getOrgBookings(params: {
  ...
  try {
    ...
    if (params.userIds !== undefined) qp.userIds = params.userIds;
    const data = await calApi(`organizations/${params.orgId}/bookings`, { params: qp });
    return ok(data);
  } catch (err) {
    return handleError("get_org_bookings", err);
  }
}

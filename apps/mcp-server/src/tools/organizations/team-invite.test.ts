...
describe("createOrgTeamInvite", () => {
  it("exports createOrgTeamInviteSchema", () => { expect(createOrgTeamInviteSchema).toBeDefined(); });
  it("returns data on success", async () => {
    mockCalApi.mockResolvedValueOnce({ status: "success" });
    const result = await createOrgTeamInvite({"orgId":1,"teamId":1});

...
describe("getTeamMemberships", () => {
	...
	it("passes pagination params", async () => {
		...
		expect(mockCalApi).toHaveBeenCalledWith("teams/1/memberships", { params: { take: 10, skip: 5 } });
	});
...
describe("createTeamInvite", () => {
	...
	it("returns data on success", async () => {
		...
		expect(result.content[0].type).toBe("text");
		expect(JSON.parse(result.content[0].text)).toEqual({ status: "success", inviteLink: "https://..." });
	});

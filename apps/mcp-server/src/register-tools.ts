...
export function registerTools(server: McpServer): void {
  ...
  server.tool("reschedule_booking", "Reschedule a booking to a new time. The new start time must be in UTC ISO 8601. Use rescheduledBy with the event owner's email (from get_me) for auto-confirmation — never fabricate emails.", rescheduleBookingSchema, rescheduleBooking);

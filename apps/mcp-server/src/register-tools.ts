...
export function registerTools(server: McpServer): void {
  ...
  server.tool("reschedule_booking", "Reschedule a booking to a new time. The new start time must be in UTC ISO 8601. rescheduledBy is only needed for confirmation-required bookings — use the event owner's email (from get_me) for auto-confirmation. Never fabricate emails.", rescheduleBookingSchema, rescheduleBooking);
  server.tool("cancel_booking", "Cancel a booking by UID. For recurring non-seated bookings, set cancelSubsequentBookings=true to also cancel all future recurrences in the series.", cancelBookingSchema, cancelBooking);
  ...
  server.tool("add_booking_attendee", "Add a new attendee to an existing booking. Required: name, email, timeZone. The email must be a real, known address — never guess or fabricate.", addBookingAttendeeSchema, addBookingAttendee);
  ...
  server.tool("get_busy_times", "Get busy times from a connected calendar. Required: dateFrom, dateTo, credentialId, and externalId. Use get_connected_calendars first to obtain credentialId and externalId — never guess these values.", getBusyTimesSchema, getBusyTimes);
